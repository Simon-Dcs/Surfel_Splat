...
def construct_list_of_attributes_2d(f_dc, f_rest, scales, rotations):
    l = ['x', 'y', 'z', 'nx', 'ny', 'nz']
...
def export_2d_gaussian(extrinsics: Float[Tensor, "4 4"],
    means: Float[Tensor, "gaussian 3"],
    scales: Float[Tensor, "gaussian 2"],
    rotations: Float[Tensor, "gaussian 4"],
    harmonics: Float[Tensor, "gaussian 3 d_sh"],
    opacities: Float[Tensor, " gaussian"],
    path: Path):
    ...
    harmonics_view_invariant = harmonics[..., 0]
    f_rest = harmonics[..., 1]
    dtype_full = [(attribute, "f4") for attribute in construct_list_of_attributes_2d(harmonics_view_invariant[..., None],
                                                                                     f_rest[..., None],
                                                                                     scales,
                                                                                     rotations
                                                                                     )]
    ...
    attributes = (
        means.detach().cpu().numpy(),
        torch.zeros_like(means).detach().cpu().numpy(),
        harmonics_view_invariant[..., None].detach().flatten(start_dim=1).contiguous().cpu().numpy(),
        f_rest[..., None].detach().flatten(start_dim=1).contiguous().cpu().numpy(),
        opacities[..., None].detach().cpu().numpy(),
        scales.log().detach().cpu().numpy(),
