...
class QKVAttentionLegacy(nn.Module):
    ...
    def forward(self, qkv):
        ...
        if self.use_cross_view_self_attn:
            # Dynamically infer the actual number of views from input shape
            # Input shape: (v*b, n, t)
            vb = qkv.shape[0]

            # Try to use n_frames if it divides vb evenly, otherwise use vb itself
            if vb % self.n_frames == 0:
                actual_v = self.n_frames
            else:
                # Fallback: assume batch_size=1, so v=vb
                actual_v = vb

            qkv = rearrange(qkv, "(v b) n t -> b n (v t)", v=actual_v)
            n_frames_used = actual_v
        else:
            n_frames_used = None
        ...
        if self.use_cross_view_self_attn:
            a = rearrange(a, "b n (v t) -> (v b) n t", v=n_frames_used)
