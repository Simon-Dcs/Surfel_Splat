...
class GaussianHead(nn.Module):
    def __init__(self, gau_in, gaussian_raw_channels):
        super().__init__()
...
class DepthPredictorMultiView(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print('Candidate ->',self.num_depth_candidates)
        # ablation settings
        # Table 3: base
        ...
        self.gau_in_channel = gau_in
        self.to_gaussian = GaussianHead(gau_in, gaussian_raw_channels)
        ...
        # for param in self.to_gaussians.parameters():
        #     param.requires_grad = True
        # for name, param in self.named_parameters():
        #     if 'to_gaussians' not in name:
        #         param.requires_grad = False
        #         #print('WOW')
        #     else:
        #         param.requires_grad = True
        ...
        # ============================================
        # DINOv2-based Monocular Depth Prediction
        # ============================================
        if use_dinov2:
            from ..backbone.dinov2_backbone import DINOv2Backbone

            # DINOv2 feature extractor
            self.dinov2_backbone = DINOv2Backbone(
                model_name=dinov2_model_name,
                freeze_backbone=freeze_dinov2
            )

            # Get DINOv2 feature dimension
            dino_feat_dim = self.dinov2_backbone.get_feature_dim()
            # Input: DINOv2 features (B, dino_feat_dim, H/14, W/14)
            # Output: Inverse depth (B, 1, H, W)

            from ..backbone.enhanced_depth_head import UltraHighCapacityDepthHead

            self.monocular_depth_head = UltraHighCapacityDepthHead(
                in_channels=dino_feat_dim,
                target_size=(256, 320)
            )
    ...
    def forward(
        ...
        # ============================================
        # DINOv2 Monocular Depth Prediction Branch
        # ============================================
        if self.use_dinov2:
            # Extract images: (b, v, 3, H, W) -> (v*b, 3, H, W)
            images = extra_info["images"]  # Already in (v*b, 3, H, W) format
            target_h, target_w = images.shape[-2:]  # Target resolution (e.g., 256, 320)

            # Step 1: Extract DINOv2 features
            dino_features = self.dinov2_backbone(images)  # (v*b, 1024, H/14, W/14)

            # Step 2: Predict inverse depth using the depth head
            pred_inv_depth_raw = self.monocular_depth_head(dino_features)  # (v*b, 1, 256, 320)

            # Step 3: Convert to depth using inverse depth space
            # Sigmoid normalizes raw output to [0, 1]
            normalized_inv_depth = torch.sigmoid(pred_inv_depth_raw)  # (v*b, 1, 256, 320)

            # Map to actual inverse depth range [1/far, 1/near]
            near_val = rearrange(near, "b v -> (v b) () () ()")  # (v*b, 1, 1, 1)
            far_val = rearrange(far, "b v -> (v b) () () ()")    # (v*b, 1, 1, 1)

            inv_near = 1.0 / far_val   # far corresponds to small inverse depth
            inv_far = 1.0 / near_val   # near corresponds to large inverse depth

            # Linear interpolation to inverse depth range
            pred_inv_depth = inv_near + normalized_inv_depth * (inv_far - inv_near)

            # Convert to depth (add eps to avoid division by zero)
            pred_depth = 1.0 / (pred_inv_depth + 1e-6)  # (v*b, 1, H, W)

            # Step 5: Generate Gaussian-related features for downstream compatibility
            ...
            # Feature upsampling (preserving original logic)
            ...
            # Use DINOv2-predicted depth instead of depth refinement
            refine_out = self.refine_unet(torch.cat(
                (images, proj_feature, normalized_inv_depth, torch.ones_like(normalized_inv_depth)), dim=1
            ))

            # Step 6: Gaussian head (unchanged)
            raw_gaussians_in = [refine_out, images, proj_feat_in_fullres]
            raw_gaussians_in = torch.cat(raw_gaussians_in, dim=1)
            raw_gaussians = self.to_gaussian(raw_gaussians_in)
            raw_gaussians = rearrange(
                raw_gaussians, "(v b) c h w -> b v (h w) c", v=v, b=b
            )

            # Step 7: Generate densities (fixed high confidence for monocular prediction)
            densities = torch.ones_like(pred_depth) * 0.9
            densities = repeat(
                densities,
            ...
            # Step 8: Reshape depths to expected format
            depths = repeat(
                pred_depth,
                "(v b) dpt h w -> b v (h w) srf dpt",
                b=b, v=v, srf=1,
            ...
            return depths, densities, raw_gaussians, raw_gaussians_in, self.gau_in_channel

        # Original Cost Volume Branch
        feat_comb_lists, intr_curr, pose_curr_lists, disp_candi_curr = (
            prepare_feat_proj_data_lists(
                features,
                intrinsics,
        ...
        feat01 = feat_comb_lists[0]
        #print('feat01 shape is ',feat01.shape)
        ...
        if self.wo_cost_volume_refine:
            ...
            #print('\n Shape is ',raw_correlation_in.shape,'\n')
        ...
        proj_feat_in_fullres = self.upsampler(torch.cat((feat01, cnn_features), dim=1))
        proj_feature = self.proj_feature(proj_feat_in_fullres)
        # print('proj_feature shape is ',proj_feature.shape)
        # print('fullres_disps shape is ',fullres_disps.shape)
        # print('pdf_max shape is ',pdf_max.shape)
        # print('extra_info["images"] shape is ',extra_info["images"].shape)
        refine_out = self.refine_unet(torch.cat(
        ...
        #raw_gaussians_2 = max_pool(raw_gaussians_in)
        raw_gaussians = self.to_gaussian(raw_gaussians_in)
        raw_gaussians = rearrange(
            raw_gaussians, "(v b) c h w -> b v (h w) c", v=v, b=b
        )
        #raw_gaussians_small = self.to_gaussians(raw_gaussians_2)
        #raw_gaussians_small = rearrange(raw_gaussians_small, "(v b) c h w -> b v (h w) c", v=v, b=b)
