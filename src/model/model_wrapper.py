...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        gaussians = self.encoder(
            batch["context"], self.global_step, False, scene_names=batch["scene"]#,visualization_dump=dump
        )
    ...
    def test_step(self, batch, batch_idx):
        ...
        # print(batch_idx,'th depth is ',output.depth.max())
        # print(batch_idx,'th depth is ',output.depth.shape)
        # Save images.
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #print('output_softmax.color shape is ',output_softmax.color.shape)
        rgb_softmax = output_softmax.color[0]
        #print('rgb_softmax',rgb_softmax.shape)
        ...
        #print('projections are ',projections.shape)
        self.logger.log_image(
            "projection",
    ...
    def render_video_generic(
        ...
        #print('gaussians_prob',gaussians_prob.covariances)
        ...
        #print('output_prob ',output_prob.depth.shape)
        images_prob = [
            vcat(rgb, depth)
        ...
        # Try to log video, but don't fail if there are issues
        try:
            visualizations = {
            ...
            if wandb.run is not None:
                ...
                wandb.init(project="Try_Train", name="name")
                wandb.log(visualizations)
        except Exception as e:
            # If wandb video logging fails, try local logging with moviepy
            print(f"Warning: Failed to log video to wandb: {e}")
            try:
                assert isinstance(self.logger, LocalLogger)
                # Save video locally using moviepy
                clip = mpy.ImageSequenceClip(list(video), fps=30)
                dir = LOG_PATH / f"video/{name}"
                dir.mkdir(exist_ok=True, parents=True)
                clip.write_videofile(
                    str(dir / f"{self.global_step:0>6}.mp4"), logger=None
                )
            except Exception as e2:
                # If local logging also fails, just skip video generation
                print(f"Warning: Failed to save video locally: {e2}. Skipping video generation.")
