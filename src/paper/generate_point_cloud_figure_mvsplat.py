...
SCENES = (
    ...
    ("5aca87f95a9412c6",50,133,10.0,[120],1.4,19),
    #("57d3409bf04c4651",10,30,6.0,[120],1.4,19),
    #("cd74b1244d112628",10,50,6.0,[110],1.4,19),
    #("84ab392d682f296b",10,100,6.0,[110],1.4,19),
    #("scannet_scene0000_00",0,2,4,6,8,10,6.0,[110],1.4,19),
    #("032dee9fb0a8bc1b90871dc5fe950080d0bcd3caf166447f44e60ca50ac04ec7",2,12,22,32,6.0,[110],1.4,19),
    #("scan38_train",2,10,6.0,[110],1.4,19),
    #("dl3dv_98c131f76b72886e98eb22397f22d2001e8a667cf91b89df4f61194756f6dea3",20,60,60,100,120,140,6.0,[110],1.4,19),
)
...
def generate_point_cloud_figure(cfg_dict):
    ...
    for idx, (scene, *context_indices, far, angles, line_width, cam_div) in enumerate(
        ...
        means = rearrange(
            gaussians.means, "() (v h w spp) xyz -> h w spp v xyz", v=NUM_VIEWS, h=h, w=w
        )
        ...
        for angle in angles:
            ...
            frustum_corners = unproject_frustum_corners(
                ...
                torch.ones((NUM_VIEWS,), dtype=torch.float32, device=device) * far / cam_div,
            )
