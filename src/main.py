...
def load_checkpoint_with_missing_keys(model, checkpoint_path):
    """
    Load a checkpoint into a model with architecture changes.
    ...
    # Print statistics about what's being loaded
    print(f"Loading checkpoint from {checkpoint_path}")
    print(f"  - Total parameters in checkpoint: {len(pretrained_dict)}")
    print(f"  - Compatible parameters: {len(compatible_dict)}")
    print(f"  - New parameters initialized randomly: {len(model_dict) - len(compatible_dict)}")
    
    # Update the model state dict with the compatible parameters
...
def train(cfg_dict: DictConfig):
    ...
    trainer = Trainer(
        ...
        strategy="ddp_find_unused_parameters_true", #"ddp" if torch.cuda.device_count() > 1 else "auto",
        callbacks=callbacks,
    ...
    if cfg.mode == "train" and checkpoint_path is not None and not cfg.checkpointing.resume:
        ...
        # model_wrapper = load_checkpoint_with_missing_keys(model_wrapper, checkpoint_path)
        # # Load the checkpoint
        # checkpoint = torch.load(checkpoint_path, map_location="cpu")
        # pretrained_dict = checkpoint["state_dict"]
        
        # # Load the compatible weights, ignore missing ones
        # model_wrapper.load_state_dict(pretrained_dict, strict=False)
